...
//      return (integer_mix(src^a[seed%8]))^(integer_mix(dst^a[(seed+1)%8]));
	  ...
	  // return (mix(src^a[seed%8]))^(mix(dst^a[(seed+1)%8])) ? (mix(src^a[seed%8])):(mix(dst^a[(seed+1)%8]));
    }
	
	/** \brief Returns the hashed value of an edge. */
    inline static size_t hash_edge2 (const std::pair<vertex_id_type, vertex_id_type>& e, const uint32_t seed = 5) {
      // a bunch of random numbers
#if (__SIZEOF_PTRDIFF_T__ == 8)
      static const size_t a[8] = {0x6306AA9DFC13C8E7,
        0xA8CD7FBCA2A9FFD4,
